...
def nhan_bai_tap(ten_bai_tap, do_kho):
    ...
    if thoi_gian_hoan_thanh < thoi_gian_duoc_cho - 200:
        print(
            "Rất tốt! Bạn hoàn thành sớm hơn "
            + str(math.floor(thoi_gian_duoc_cho - thoi_gian_hoan_thanh))
            + " giây so với yêu cầu."
        )
    elif (
        thoi_gian_hoan_thanh >= thoi_gian_duoc_cho - 200
        and thoi_gian_hoan_thanh <= thoi_gian_duoc_cho
    ):
